...
//New Code
  ...
  void _parseSpeech(String words) {
    List<String> parts = words.split(' ');
    print("eytu word $parts");
    int quantityIndex = -1;

    for (int i = 0; i < parts.length; i++) {
      if (parts[i].toLowerCase().similarityTo("quantity") > 0.5) {
        quantityIndex = i;
      }
    }

    if (quantityIndex != -1 && quantityIndex + 1 < parts.length) {
      String product = parts.sublist(0, quantityIndex).join(' ');
      String parseQuantity = parts.sublist(quantityIndex + 1).join(' ');

      // Split parseQuantity into two parts based on a separator
      List<String> quantityParts = parseQuantity.split(',');

      try {
        print("parsequnatity: $quantityParts[0]");
        if (isNumeric(quantityParts[0])) {
          quantityNumeric = double.parse(quantityParts[0]);
          String unitOfQuantity = quantityParts.length > 1 ? quantityParts[1].trim() : '';

          productNameController.text = product;
          quantityController.text = quantityNumeric.toString();
          // Now you have both quantityNumeric and unitOfQuantity
          ...
          showDialog(
            ...
            builder: (BuildContext context) {
              return AlertDialog(
                title: Text("Error"),
                content: Text("'$quantityParts' is not a valid number."),
                actions: <Widget>[
                  ElevatedButton(
                    child: Text("OK"),
                    onPressed: () {
